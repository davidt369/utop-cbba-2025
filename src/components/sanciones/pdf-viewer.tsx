...
// Componente para mostrar un botón/link de PDF para sanciones
...
export function PDFLink({ sancionId, pdfFileName, className = "" }: PDFLinkProps) {
    const [isDownloading, setIsDownloading] = useState(false);

    const baseUrl = process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000/api';
    const pdfUrl = `${baseUrl}/auth/sanciones/${sancionId}/pdf`;
    const displayName = pdfFileName || `PDF-${sancionId}`;

    const downloadPDF = async () => {
        try {
            setIsDownloading(true);

            console.log('🔍 Descargando PDF de sanción:', { sancionId, pdfUrl, pdfFileName });

            if (!sancionId || sancionId <= 0) {
                alert('ID de sanción inválido');
                return;
            }
    ...
    // No renderizar si no hay sancionId válido
    if (!sancionId || sancionId <= 0) {
        return null;
    }
